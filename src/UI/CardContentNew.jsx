...
const CardContentNew = ({ classNameCard, icon, title, description }) => {
    ...
    >
      <div className="flex pl-6 -top-7 absolute">
        <div className="p-4 rounded-lg border-grey-15 bg-orange-90 border-2">
          {IconComponent ? (
            <IconComponent className="w-5 h-5 2xl:w-6 2xl:h-6 text-black" />
          ) : (
            <div className="w-5 h-5 bg-gray-300" /> // fallback jika tidak ada icon
          )}
        </div>
      </div>
